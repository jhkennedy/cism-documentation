...
%WHL - not sure if the following still applies
%The netCDF functionality is split up so that other subsystems of the model 
...
%Have a look at the implementation of the \texttt{eis} climate driver.
...
%% As of 9/15/14, Matt H. says this not true/working.
...
\begin{table}[htbp]
 \begin{center}
  \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|l|p{10cm}|}
    ...
    \hline
%% MJH: this no longer works - now glide_setup.F90 controls this    \texttt{hot} & Set to 1 if the variable should be saved for hot--starting (i.e., restarting) the model (implies \texttt{load}=1)\\
%%    \hline
