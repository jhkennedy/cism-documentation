...
%%%% Options
    ...
    \hline
    \multicolumn{2}{|p{0.95\textwidth}|}{Parameters set in this section determine how various components of the ice sheet model are treated.  Configuration number options with a * after them are specific to the higher-order dycores (e.g., Glissade). Options with a ? after them are working, but are currently not scientifically supported, and are therefore for use at your own risk. Options with a ! after them are in development and will be supported in future code releases. \textbf{Steve: At the moment, I don't think it is clear below which options work w/ the parallel code and which do not. We may need to introduce some clear way of denoting that.} }\\
    \hline
%% dycore
    \texttt{dycore} & 
...
%% restart
 ...
 %%%% HIGHER-ORDER OPTIONS
    ...
    \hline
    \multicolumn{2}{|p{0.95\textwidth}|}{Options set in this section determine how various components of the higher-order extensions to the ice sheet model (e.g., Glissade) are treated. Defaults are indicated in bold. These options have no effect on the Shallow-Ice (Glide) dycore. In this section, options with a ? after them are working, but are currently not scientifically supported (and are therefore for use at your own risk). Options marked with a * apply to a serial build of the model only. Options marked with a ! are under development and will be fully supported in future versions of the code (hence, these are also use at your own risk).}\\
    \hline
%% which_ho_nonlinear
