...
%%%% TIME
    ...
    \hline
    \multicolumn{2}{|p{0.95\textwidth}|}{Configure time steps, etc. {\bf TODO: Address/remove this note:} Update intervals should probably become absolute values rather than related to the main time step when we introduce variable time steps. \textbf{Steve: I don't actually know what the question is here or what this note is supposed to be about.}}\\
    \hline
    ...
    Maybe we need to look into this to figure out what it does and if it works? Matt, my guess would be that Pat's profiling code is superior to this.}\\
...
%%%% Options
    ...
    \hline
    \multicolumn{2}{|p{0.95\textwidth}|}{Parameters set in this section determine how various components of the ice sheet model are treated.  Configuration number options with a * after them are specific to the higher-order dycores (e.g., Glissade). Options with a ? after them are working, but are currently not scientifically supported, and are therefore for use at your own risk. Options with a ! after them are in development and anticipated as being supported in future code releases.}\\
    \hline
%% dycore
    \texttt{dycore} & 
...
%% evolution
    ...
    \begin{tabular}[t]{lp{0.85\linewidth}}
      ...
      5* & evolve without changing ice thickness. This can be useful when it is desired to run the model with a fixed geometry, e.g. during a temperature spinup. Note that on each time step this option evolves the geometry and tracers using incremental remapping and then resets geometry to its old value, so that tracer advection can still occur within a fixed geometry.  This evolution scheme is therefore still subject to the advective CFL condition.\\
    \end{tabular}\\
%% temperature
    \texttt{temperature} & 
    \begin{tabular}[t]{lp{0.85\linewidth}}
      ...
      3! & prognostic temperature calculation using enthalpy formulation \\
    \end{tabular}\\
%% temperature init
    \texttt{temp\_init} & 
    \begin{tabular}[t]{lp{0.85\linewidth}}
...
%% flow law
    ...
    \begin{tabular}[t]{lp{0.85\linewidth}}
      ...
      1 & temp. dependent, \citet{PatersonBudd:1982} (temp.=-5degC)\\
      2 & temp. dependent, \citet{PatersonBudd:1982} (function of variable temp.)\\
    \end{tabular}\\
%% basal water
    \texttt{basal\_water} & 
    \begin{tabular}[t]{lp{0.85\linewidth}}
...
%% slip coefficient
    \texttt{slip\_coeff} & 
        slip coefficient (for Glide \textit{only}) \\ &
    \begin{tabular}[t]{lp{0.85\linewidth}}
...
%% restart
 ...
 %%%% HIGHER-ORDER OPTIONS
    ...
    \hline
    \multicolumn{2}{|p{0.95\textwidth}|}{Options set in this section determine how various components of the higher-order extensions to the ice sheet model (e.g., Glissade) are treated. Defaults are indicated in bold. These options have no effect on the Shallow-Ice (Glide) dycore. In this section, options marked with a * apply to a serial build of the model only and
    options marked with a ! are under development and will be fully supported in future versions of the code (use now at your own risk!).}\\
    \hline
%% which_ho_nonlinear
...
%\\ &
    \begin{tabular}[t]{lp{0.85\linewidth}}
      ...
      0* & Solve sparse linear system using SLAP library with incomplete LU-preconditioned biconjugate gradient method\\
...
Bill, do you know if we are calculating the necessary info (strain-rates / stresses) from Glissade output for this option to be working as advertised?} \\ &
    \begin{tabular}[t]{lp{0.85\linewidth}}
...
%% which_ho_babc
    \texttt{which\_ho\_babc} & 
        Implementation of basal boundary condition in higher-order dycore (Calculation of `beta' field.) \\ &
    \begin{tabular}[t]{lp{0.85\linewidth}}
      0 & constant value \\
      ...
      {\bf 4} & no slip everywhere in domain (`beta' set to very
          large value)\\
      5 & read map of `beta' in from .nc input file using standard i/o \\
...
%% which_ho_approx
    \texttt{which\_ho\_approx} &
     Which reduced-order, Stokes-flow approximation to use with Glissade dycore \\ &
    \begin{tabular}[t]{lp{0.85\linewidth}}
...
%% which_ho_gradient
    ...
    \begin{tabular}[t]{lp{0.85\linewidth}}
      0 & Centered gradient \\
      {\bf 1} & Upstream gradient {\bf TODO: Make sure this has been set as the default in the code (currently default is 0). Steve: Can we clarify if / that this only
      applies at margins? That is, in the ice sheet center, the scheme defaults to using of centered gradients?}\\
    \end{tabular}\\  
...
%%%% PARAMETERS
    ...
    \hline
    \multicolumn{2}{|p{0.95\textwidth}|}{Set values for various parameters.  Parameters with a * after them are specific to the higher-order dycores (e.g., Glissade).}\\
    \hline
    ...
    \texttt{flow\_factor} & (real) the flow law is enhanced with this factor (default = 3.0) \textbf{Steve: Should this be 1? Is seems crazy to set the default for this to anything
    other than unity. We currently specify flow\_factor = 1 in every standard higher-order test case. Is it 3 by default because the SIA code assumes that and setting it to 
    something else will throw those results off? Looks like all of the EISMINT 1 and 2 tests are also explicitly setting this to 1 in the config files!} \\
...
%%%% ISOSTASY
    ...
    \hline
    \multicolumn{2}{|p{0.95\textwidth}|}{Options related to isostasy model.  Ignored unless \texttt{isostasy} = 1. Options marked with a * currently work in both serial and parallel.
    \textbf{Steve: I tested these out and compared w/ older versions of the code (this was an old to-do item). As far as I know, all options ``work", but only the combinations using
     the local lithosphere (no flexural rigidity) work in parallel.}} \\  
    \hline
...
%%%% PROJECTION
    ...
    referred to Snyder J.P. (1987) \emph{Map Projections - a working manual.} USGS 
        Professional Paper 1395.    {\bf TODO: I am assuming this works.  Code is not in glide\_setup.F90.  
        Appears to be in glimmap\_printproj() in ../libglimmer/glimmer\_map\_init.F90. Steve: I don't know anything
        about this ... Bill?}}\\
    \hline
