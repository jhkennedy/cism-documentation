...
\begin{enumerate}
\item download a {\it released} version from the \href{http://glimmer.forge.nesc.ac.uk}{GLIMMER-CISM website}\footnote{\texttt{http://glimmer.forge.nesc.ac.uk}}, or \textbf{SP: We'll need to update this link}
\item download the latest developers' version from \href{http://forge.nesc.ac.uk/}{NeSCForge} using \href{http://www.gnu.org/software/cvs/}{CVS}. \textbf{SP: Do we want to support this or just support the download-a-tag option?}
\end{enumerate}
...
\textbf{SP: confirm that we need libtool, automake, autoconf ... if not, then just install Cmake?}
...
\textbf{SP: add some notes here on what you should see happening for a successful build?}

When the build completes, you can check for the executable driver by typing \texttt{ls cism\_driver} from within the \texttt{./builds/mac-gnu/} directory. The file \texttt{cism\_driver*} there is the executable you will link to when running the model (e.g., for the test cases described in section XX). \textbf{SP: add link to test case description section of documentation here}
