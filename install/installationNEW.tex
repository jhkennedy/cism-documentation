...
% =====================================
% =====================================
This section describes setting up supporting software and CISM in a Linux environment.
...
installing new packages.  If you are working on a shared machine without
...
% =====================================
\subsection{Basic supporting software needed for CISM}
% =====================================

A number of software tools are necessary before performing a basic installation of CISM.
Most systems are likely to have some or all of these packages already installed, 
but these instructions are intended to be complete for a fresh installation of Ubuntu 12.10.
Note that the commands below will execute safely if the packages are already installed.
Additional tools are needed for a parallel installation using the Trilinos solver libraries, 
which are described in another section below.

\begin{enumerate}
% =====================================
\item{\texttt{git} - version control software

If you intend to download the CISM code as a git repository, you will need the \texttt{git} package installed.
If you download a zipped archive of the code, this step can be skipped.
...
% =====================================
\item{\texttt{Fortran and C compilers} - needed for compiling code
...
% =====================================
\item{\texttt{Build tools} - miscellaneous tools for compiling code
...
% =====================================
\item{\texttt{NetCDF} - network Common Data Form libraries (a
 machine-independent format for representing scientific data)

This is required by CISM.

Install NetCDF with:

\texttt{sudo apt-get install libnetcdf-dev}

Additionally, it is recommended to install some useful tools for working with NetCDF files:

\texttt{sudo apt-get install netcdf-bin ncview nco}
}

% =====================================
\item{\texttt{python} - scripting language
...
\begin{mdframed}
Optional: Installing \texttt{netCDF4} python module.
...
% =====================================
\subsection{Building serial CISM}
% =====================================

For convenience, a build script for serial CISM is included \texttt{builds/linux-gnu-cism}.
From that directory, run:

\texttt{ source linux-gnu-cism-cmake-serial}

This build script should work for most standard Linux setups.  However, if you encounter
an error, you may need to modify some details, such as the location of your NetCDF libraries 
...
Once the configuration step completes successfully, you can compile the code with:
...
% =====================================
\subsection{Supporting software needed for parallel CISM}
% =====================================

\begin{enumerate}
% =====================================
\item{\texttt{MPI} - message passing interface libraries and compilers for compiling and running parallel code

MPI libaries and compilers are necessary for compiling parallel CISM.  There are a few options
available, but OpenMPI is the most common implementation and one that has been 
...
% =====================================
\item{\texttt{Trilinos} - solver libraries

TODO: How to build Trilinos
...
% =====================================
\subsection{Building parallel CISM}
% =====================================

The procedure for building parallel CISM is nearly identical to the serial build (above).
The build script for parallel CISM is located in \texttt{builds/linux-gnu-cism}.
From that directory, run:

\texttt{ source linux-gnu-cism-cmake}

This build script should work for most standard Linux setups.  However, if you encounter
an error, you may need to modify some details, such as the location of your NetCDF libraries 
or your compiler names.  Other errors you might encounter may indicate that some of
the supporting software (above) is missing.  Common problems here have to do with 
MPI, for example, the location of the MPI include or library directories.
