...
but as with ``Glimmer'' the acronym has never been widely used.) Glissade is the successor to the shallow-ice
...
approximations.  The most accurate and complex scheme is a 3D first-order Blatter-Pattyn solver, 
described in Section (cite section).  
Glissade can also solve the shallow-shelf equations and the shallow-ice equations.
...
described below in Section (cite section): 
incremental remapping (the more complex and accurate scheme) and first-order upwind. These schemes
transfer both mass and internal energy in the horizontal direction, followed by a vertical remapping step
that restores the vertical grid to a sigma coordinate.
...
transport scheme, and a separate vertical temperature solver handles vertical diffusion and internal dissipation.
The vertical solver is local; each ice column calculation is independent of every other column calculation.
